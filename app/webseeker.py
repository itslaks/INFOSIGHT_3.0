...
# Removed groq import - using centralized router
...
try:
    ...
    class Config:
        VIRUSTOTAL_API_KEY = AppConfig.VIRUSTOTAL_API_KEY
        IPINFO_API_KEY = AppConfig.IPINFO_API_KEY
        ABUSEIPDB_API_KEY = AppConfig.ABUSEIPDB_API_KEY
...
Config.ALLOWED_SCAN_TYPES = ['quick', 'comprehensive', 'stealth']
Config.PORT_RANGES = {
...
try:
    ...
    logger.info("✓ LLM router available for WebSeeker")
...
try:
    from utils.llm_logger import log_llm_status
    log_llm_status("WebSeeker")
except:
    pass
...
class AIAnalyzer:
    ...
    def generate_website_description(target, website_info, scan_results):
        """Enhanced AI-powered website analysis using LLM router (Groq with Ollama fallback). Returns (description, model_used)"""
        model_used = "fallback"
        
        if not website_info.get('success'):
            return (f"Unable to analyze {target} - website may be inaccessible, blocking automated requests, or down for maintenance. Manual review required.", model_used)
        ...
        if not has_content:
            return (f"{target} - Website is accessible but contains minimal content. May be under construction or require JavaScript rendering.", model_used)
        
        # Try LLM router if configured
...
Write naturally in plain text - no formatting, no JSON, no markdown."""
            ...
            try:
                result = generate_text(
                    ...
                    system_prompt="You are a web security analyst. Analyze websites and provide detailed, professional descriptions.",
                    temperature=0.7,
                    max_tokens=500
                )
                description = result.get("response", "").strip()
                model_used = result.get("source", "local")
                ...
                if len(description) >= 100 and 'unable' not in description.lower() and target not in description[:50]:
                    # Store model_used for return
                    description_with_model = (description, model_used)
                    return description_with_model
                ...
                try:
                    ...
                    from utils.llm_logger import log_llm_error, log_llm_fallback, log_llm_request, log_llm_success
                    LOCAL_LLM_AVAILABLE = check_ollama_available()
                except ImportError:
                ...
                if LOCAL_LLM_AVAILABLE and any(keyword in error_str for keyword in [
                    ...
                    log_llm_error("WebSeeker", "cloud", llm_error, fallback=True)
                    log_llm_fallback("WebSeeker", "cloud", "local")
                    logger.warning(f"LLM error for {target}: {llm_error}. Trying local Ollama...")
                    
                    try:
                        log_llm_request("WebSeeker", "local", len(prompt))
                        start_time = time.time()
                        local_result, success = generate_with_ollama(
                            prompt,
                            system_prompt="You are a web security analyst. Analyze websites and provide detailed descriptions.",
                            temperature=0.7,
                            max_tokens=500
                        )
                        ...
                        if success and local_result:
                            log_llm_success("WebSeeker", "local", len(local_result), latency_ms)
                            description = local_result.strip()
                            description = description.replace('```', '').replace('**', '').replace('*', '').strip()
                            description = ' '.join(description.split())
                            
                            if len(description) >= 100:
                                logger.info(f"✓ Used local Ollama for {target}")
                                return (description, "local")
                        else:
                            log_llm_error("WebSeeker", "local", Exception("Local LLM returned empty"), fallback=False)
                    except Exception as ollama_error:
                        log_llm_error("WebSeeker", "local", ollama_error, fallback=False)
                        logger.warning(f"Local Ollama fallback also failed for {target}: {ollama_error}")
                
                logger.warning(f"LLM API error for {target}: {str(llm_error)}")
        
        # Fallback to enhanced rule-based analysis
        fallback_desc = AIAnalyzer._enhanced_fallback_analysis(target, website_info)
        return (fallback_desc, "fallback")
    ...
    def generate_executive_summary(scan_results):
        ...
        result = AIAnalyzer.generate_website_description(target, website_info, scan_results)
        # Handle new return format (description, model_used) or old format (description)
        if isinstance(result, tuple):
            website_description, model_used = result
        else:
            website_description = result
            model_used = "groq"
        ...
        return {
            ...
            "model_used": model_used,  # 'groq', 'local', or 'fallback'
...
class SecurityScanner:
    ...
    def get_ip_info(self, target):
        ...
        try:
            ...
            # ASN lookup
            try:
                asn_response = requests.get(f"https://ipapi.co/{ip}/json/", timeout=5, verify=False)
...
@validate_request({
    "domain": {
        ...
        "pattern": None  # Domain validation handled by scanner
    },
    "scanType": {
        "type": "string",
        "required": False,
        "max_length": 50,
        "allowed_values": Config.ALLOWED_SCAN_TYPES
    }
}, strict=True)
def analyze():
    """
...
@rate_limit_api(requests_per_minute=20, requests_per_hour=200)  # More lenient for quick checks
def quick_check():
    """
    ...
    try:
        ...
        target = InputValidator.validate_string(
            target, 'domain', max_length=253, required=True
        )
