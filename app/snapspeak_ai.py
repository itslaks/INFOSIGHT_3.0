...
# ==================== REMOVED: unused _safe_import_transformers block ====================
# BlipForConditionalGeneration / BlipProcessor loading is handled entirely by
# utils.vision_analyzer via its own lazy-load mechanism.  Keeping dead references
# here only caused confusion and risked double-loading heavy models.
# ========================================================================================
...
try:
    ...
    from utils.llm_logger import (
        log_llm_status, log_llm_request, log_llm_success,
        log_llm_error, log_llm_fallback, log_processing_step,
    )
...
try:
    from utils.vision_analyzer import (
        ...
        cloud_caption, local_caption, detect_objects, extract_colors,
    )
...
# Maximum dimension for heavy numpy operations – images larger than this are
# downsampled BEFORE processing to avoid excessive RAM consumption.
MAX_PROCESSING_DIMENSION = 2000

# BLIP and vision capabilities are provided by utils.vision_analyzer (lazy-loaded)
BLIP_AVAILABLE = VISION_ANALYZER_AVAILABLE

# ==================== LAZY LOADERS ====================

# DeepFace: lazy import to avoid heavy Keras/TF load at module import.
# Initialised to None so callers can distinguish "not yet tried" from "not available".
DEEPFACE_AVAILABLE = None   # None = not yet probed; True/False after first call
_deepface_module = None


def get_deepface():
    """Return deepface module (lazy-loaded), or None if unavailable."""
    global _deepface_module, DEEPFACE_AVAILABLE
    if DEEPFACE_AVAILABLE is not None:          # already probed
        return _deepface_module
    try:
        import importlib
        _deepface_module = importlib.import_module('deepface')
...
# OCR: lazy import to avoid pytesseract loading at module import.
# Initialised to None so API responses accurately reflect first-call discovery.
OCR_AVAILABLE = None    # None = not yet probed; True/False after first call
_pytesseract = None


def get_pytesseract():
    """Return pytesseract module (lazy-loaded), or None if unavailable."""
    global _pytesseract, OCR_AVAILABLE
    if OCR_AVAILABLE is not None:               # already probed
        return _pytesseract
    try:
        import importlib
...
def _clamp_image_for_processing(image: Image.Image) -> Image.Image:
    """
    Return a (possibly resized) copy of *image* suitable for heavy numpy
    operations.  Images whose longest side exceeds MAX_PROCESSING_DIMENSION
    are downsampled using Image.thumbnail so that aspect ratio is preserved.
    The caller's original object is never mutated.
    """
    if max(image.size) > MAX_PROCESSING_DIMENSION:
        img_copy = image.copy()
        img_copy.thumbnail((MAX_PROCESSING_DIMENSION, MAX_PROCESSING_DIMENSION), Image.LANCZOS)
        return img_copy
    return image
        ...
        if LLM_ROUTER_AVAILABLE:
            try:
                ...
                llm_result = generate_text(
                    ...
                    system_prompt=(
                        "You are a professional image forensics expert. Provide clear, factual, "
                        "and detailed visual descriptions without making unverifiable assumptions "
                        "about identity or intent."
                    ),
                    temperature=0.6,
                    max_tokens=220,
                )
                ...
                if llm_result and llm_result.get('response'):
                    ...
                    logger.debug("LLM router did not return a detailed caption; using base caption only")
...
def deep_metadata_extraction(image, image_bytes):
    ...
    try:
        ...
        use_cases['basic_info'] = (
            "Essential for determining image compatibility, storage requirements, "
        ...
        # Camera Settings
        camera_tags = [
            'Image Make', 'Image Model', 'EXIF LensModel', 'EXIF FocalLength',
        ...
        if metadata['camera_settings']:
            use_cases['camera_settings'] = (
                "Recreate similar photos, verify camera authenticity, analyse photography technique"
            )
        ...
        if 'icc_profile' in image.info:
            ...
            use_cases['color_profile'] = (
                "Ensure accurate colour reproduction, detect professional vs amateur photos"
            )

        # Compression Analysis
        if image.format == 'JPEG':
            try:
                # FIX: use getattr to avoid crash when quantization attribute is absent
        ...
        # Entropy Analysis (uses downsampled copy to bound RAM)
        if image.mode == 'RGB':
            img_small = _clamp_image_for_processing(image)
            img_array = np.array(img_small)
            hist = cv2.calcHist(
                [img_array], [0, 1, 2], None,
        ...
        traceback.print_exc()
        return {'metadata': {}, 'use_cases': {}, 'error': str(e)}
...
def advanced_steganography_detection(image):
    """
    Improved multi-method steganography detection.
    Input image is downsampled when needed to prevent excessive RAM use.
    """
    try:
        # Downsample before heavy numpy operations
        image = _clamp_image_for_processing(image)
        ...
        chi_threshold = 2000 if total_pixels > 1_000_000 else 1500
        ...
        hist_threshold = 1e11 if total_pixels > 1_000_000 else 5e10
...
def enhanced_color_analysis(image):
    """
    Advanced colour analysis with colour theory insights.
    Uses a downsampled copy for KMeans to avoid excessive RAM use.
    """
    try:
        if image.mode != 'RGB':
            image = image.convert('RGB')

        # Downsample before KMeans
        img_for_kmeans = _clamp_image_for_processing(image)

        thumb = img_for_kmeans.copy()
        thumb.thumbnail((IMAGE_RESIZE_DIMENSION, IMAGE_RESIZE_DIMENSION))
        pixels = np.array(thumb).reshape(-1, 3)

        # FIX: use n_init="auto" for sklearn future-compatibility
        kmeans = KMeans(n_clusters=COLOR_CLUSTER_COUNT, random_state=42, n_init="auto")
        kmeans.fit(pixels)
        ...
        logger.error(f"Error in colour analysis: {str(e)}")
        traceback.print_exc()
        return {'colors': [], 'error': str(e)}


def determine_color_scheme(colors):
    """Determine overall colour scheme"""
    if not colors:
        return 'Unknown'
...
def enhanced_face_detection(image):
    ...
    try:
        ...
        deepface_module = get_deepface()
        if deepface_module is not None:
            try:
                import tempfile
                # FIX: use delete=True and explicit close to prevent descriptor leaks
                ...
                try:
                    faces = deepface_module.DeepFace.extract_faces(
                        tmp_path,
                    ...
                    logger.debug(f"  DeepFace detected {len(face_locations)} faces")
                finally:
                    try:
                        os.unlink(tmp_path)
                    except Exception:
...
def analyze_face_with_gemini(image):
    """Advanced face analysis using LLM router (Groq/Ollama)"""
    if not GROQ_CONFIGURED:
        return None

    try:
        prompt = """Analyse any faces in this image based on the description I will give you. Provide:
...
IMPORTANT: Do NOT attempt to identify specific individuals by name. Focus on observable characteristics only."""
        ...
        description = (
            "The previous step detected one or more faces in the image. "
            "Provide only high-level, non-identifying observations."
        )
        ...
        llm_result = generate_text(
            ...
            task_type="image_reasoning",
            system_prompt=(
                "You are an expert facial analysis AI. Provide detailed, structured observations "
                "about faces without identifying specific people."
            ),
            temperature=0.7,
            max_tokens=1024,
            prefer_local=True,
        ...
        if llm_result and llm_result.get('response'):
            return {
                'analysis': llm_result['response'].strip(),
...
def detect_ai_generation(image):
    ...
    Uses a downsampled copy to bound RAM usage.
    """
    try:
        # Downsample before heavy numpy operations
        image = _clamp_image_for_processing(image)
        ...
        high_freq_ratio = (
            np.sum(magnitude_spectrum > np.percentile(magnitude_spectrum, 90))
        ...
        if len(img_array.shape) == 3:
            ...
            if color_std < 10:
                results['indicators'].append('Uniform colour distribution')
...
def generate_caption(image):
    """Generate image caption using lazy local BLIP or cloud fallback"""
    try:
        try:
            caption = local_caption(image)
            if caption:
                return caption
        except Exception:
            pass
...
def analyze_image():
    ...
    try:
        ...
        if vision_analysis:
            ...
            logger.debug(f"Vision analysis complete (source: {vision_analysis.get('analysis_source', 'unknown')})")
        else:
            caption = generate_caption(image)

        metadata_result = deep_metadata_extraction(image, image_bytes)
        logger.debug(f"Metadata extracted: {metadata_result.get('sections_found', 0)} sections")
        ...
        logger.debug(f"Stego check: {steg_result['overall_risk']} risk")
        ...
        logger.debug(f"Face detection: {face_data.get('count', 0)} faces found")

        ai_detection = detect_ai_generation(image)

        processing_time = time.time() - start_time
        logger.info(f"Analysis complete in {processing_time:.2f}s — {file.filename}")

        analysis_results = {
            'basic_caption': caption,
            'vision_analysis': vision_analysis,
            'gemini_analysis': vision_analysis,   # backward-compat field
            ...
            'gemini_enabled': GROQ_CONFIGURED,     # backward-compat field
            'deepface_available': bool(DEEPFACE_AVAILABLE),
        }

        return jsonify(convert_to_json_serializable(analysis_results))
...
def perform_ocr(image, languages="eng"):
    ...
    try:
        if image.mode not in ['L', 'RGB']:
            image = image.convert('RGB')
        text = pyt.image_to_string(image, lang=languages, config='')
        return {
            'text': text.strip(),
...
def compute_technical_quality(image):
    """Compute basic no-reference quality metrics (sharpness, noise, dynamic range)."""
    try:
        if image.mode != 'L':
        ...
        return {
            ...
            'piqe_score': None,
            'implementation_note': (
                'Classical metrics implemented; BRISQUE/NIQE/PIQE reserved for future ML models.'
            ),
        }
    except Exception as e:
...
def estimate_prnu_signature(image):
    ...
    try:
        ...
        return {
            ...
            'approximate': True,
            'note': (
                'Approximate PRNU-like fingerprint; robust matching requires dedicated service and database.'
            ),
        }
    except Exception as e:
...
def dct_stego_analysis(image):
    ...
    try:
        ...
        return {
            ...
            'note': 'Placeholder DCT analysis; full F5/OutGuess detection requires specialised models.',
        }
    except Exception as e:
    ...
    try:
        ...
        if gps_info:
            location_details = {
                ...
                'message': (
                    'Integrate with external geocoding API (e.g., OpenStreetMap, Google Maps) '
                    'for full address/elevation.'
                ),
            }
...
def api_edit_history():
    ...
    try:
        ...
        return jsonify(convert_to_json_serializable({
            ...
            'note': (
                'Edit history reconstruction is heuristic; for legal-grade timelines '
                'integrate with dedicated forensic tools.'
            ),
        }))
    except Exception as e:
...
def api_stego_deep_scan():
    ...
    try:
        ...
        if dct.get('supported') and dct.get('qtables_present'):
            indicators.append('DCT/Quantisation Anomaly (heuristic)')
        ...
        return jsonify(convert_to_json_serializable({
            ...
            'ml_classifier': {
                'available': False,
                'note': (
                    'Deep learning stego classifier is not bundled; '
                    'integrate external model/service here.'
                ),
            },
        }))
    except Exception as e:
    ...
    try:
        ...
        return jsonify(convert_to_json_serializable({
            ...
            'automatic_decoding': {
                'attempted': False,
                'note': (
                    'Automatic decoding (base64/compressed payloads) can be added '
                    'on top of raw bytes.'
                ),
            },
        }))
...
def api_stego_tool_identification():
    ...
    try:
        ...
        return jsonify(convert_to_json_serializable({
            ...
            'note': (
                'Signature database is minimal; extend with known stego tool fingerprints '
                'for higher accuracy.'
            ),
        }))
    except Exception as e:
...
def api_reverse_search_multi_engine():
    ...
    return jsonify(convert_to_json_serializable({
        ...
        'integration_note': (
            'Wire this endpoint to external reverse image APIs / CLIP-based search service. '
...
def api_reverse_search_provenance():
    ...
    return jsonify(convert_to_json_serializable({
        ...
        'provenance_timeline': [],
        'note': (
            'Connect to multi-engine reverse search results to populate '
            'first-appearance dates and usage timelines.'
        ),
    }))
    ...
    for i in range(len(results)):
        ...
        for j in range(i + 1, len(results)):
            if 'hashes' not in results[j]:
                continue
            d = hamming(
                results[i]['hashes'].get('average_hash'),
                results[j]['hashes'].get('average_hash'),
            )
            if d is not None:
                pairs.append({
...
def api_reverse_search_copyright():
    ...
    return jsonify(convert_to_json_serializable({
        ...
        'copyright_matches': [],
        'note': (
            'Integrate with stock-photo / copyright registries (Getty, Shutterstock, etc.) '
            'to populate matches.'
        ),
    }))
    ...
    try:
        ...
        return jsonify(convert_to_json_serializable({
            ...
            'yolo_sam_3dpose': {
                'available': False,
                'note': (
                    'Connect to YOLOv8/SAM/pose-estimation service for full advanced object detection.'
                ),
            },
        }))
    except Exception as e:
...
def api_vision_document_parse():
    ...
    return jsonify(convert_to_json_serializable({
        ...
        'note': (
            'For robust key-value extraction and table parsing, '
            'integrate with specialised OCR/vision models.'
        ),
    }))
...
def api_blockchain_c2pa_verify():
    ...
    return jsonify({
        ...
        'credentials': None,
        'note': (
            'Integrate with a C2PA/CAI verification library or service '
            'to extract and verify content credentials.'
        ),
    })
...
def api_blockchain_nft_check():
    ...
    return jsonify(convert_to_json_serializable({
        ...
        'nft_matches': [],
        'note': (
            'Connect to blockchain indexers (e.g., Ethereum/Polygon NFT APIs) '
            'to look up ownership and provenance.'
        ),
    }))
...
def api_blockchain_digital_signature():
    ...
    return jsonify({
        ...
        'note': (
            'Wire this endpoint to a signature-extraction and '
...
def api_compare_visual_diff():
    ...
    try:
        ...
        return jsonify({
            ...
            'note': (
                'Raw heatmap PNG bytes are generated server-side; '
                'expose via separate download endpoint if needed.'
            ),
        })
    except Exception as e:
    ...
    for i in range(len(imgs)):
        ...
        for j in range(len(imgs)):
            if i == j or 'hashes' not in imgs[i] or 'hashes' not in imgs[j]:
                ...
                d = hamming(
                    imgs[i]['hashes'].get('average_hash'),
                    imgs[j]['hashes'].get('average_hash'),
                )
...
def api_privacy_pii_detect():
    ...
    return jsonify(convert_to_json_serializable({
        ...
        'note': (
            'For production-grade PII detection, integrate regex/rule-based + ML entity recognisers.'
        ),
    }))
...
def api_privacy_auto_redact():
    ...
    return jsonify(convert_to_json_serializable({
        ...
        'note': (
            'Apply blurring/redaction on client or downstream processor based on these regions.'
        ),
    }))
...
def api_quality_professional_report():
    ...
    aesthetic = compute_aesthetic_score(
        image, color_analysis=color_data, object_analysis=tech, faces=faces
    )
...
def api_batch_upload_folder():
    ...
    for f in files:
        try:
            ...
            results.append({'filename': f.filename, 'hashes': hashes, 'quality': quality})
        except Exception as e:
            results.append({'filename': f.filename, 'error': str(e)})
    ...
    return jsonify(convert_to_json_serializable({
        ...
        'images': results,
        'note': (
            'Synchronous batch summary; wire this to a task queue and DB for large async runs.'
        ),
    }))
...
def api_batch_check_progress():
    ...
    return jsonify({
        ...
        'progress': 0.0,
        'note': (
            'Implement persistent batch tracking (e.g., Redis + worker queue) '
            'to support long-running analyses.'
        ),
    })
...
def api_batch_results(batch_id):
    return jsonify({
        ...
        'results': [],
        'note': (
            'Batch results storage not yet wired; connect to DB where batch summaries are persisted.'
        ),
    })
...
def api_export_forensic_package():
    ...
    return jsonify(convert_to_json_serializable({
        ...
        'note': (
            'Integrate with archive/zip creation to produce downloadable forensic packages.'
        ),
    }))
...
def api_visualize_heatmap():
    ...
    try:
        ...
        return jsonify({
            ...
            'note': (
                'Hook this endpoint to model attention maps for full '
                '"what the AI looked at" visualisations.'
            ),
        })
    except Exception as e:
...
def api_visualize_timeline():
    ...
    return jsonify(convert_to_json_serializable({
        'timeline': timeline,
        'note': 'Render this timeline on the front-end as an interactive visualisation.',
    }))


# ============================================================
# STANDALONE DEV ENTRY POINT (removed from production path)
# ============================================================
